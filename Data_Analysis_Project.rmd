...
results_table_2 = df_table
knitr::kable(df_table)
```

> All three of the AIC models have the same RMSE values and adjusted R-squared. We decided to  select the backward_AIC_int model, as it is easier to explain the interaction between location and all other predictors, rather than just some of these interactions.

```{r colton_coda_1, echo = FALSE}
backward_AIC_int$call
hist(resid(backward_AIC_int), main = "Histogram for \nAIC backward using\n Additive Interactions")
get_plot('Fitted VS Residuals plot\nof AIC backward using\nAdditive and Interactions', 'QQ plot of AIC backward\nusing Additive and Interactions',
         backward_AIC_int)

```

As per the above Fitted VS Residual plots it does not seems follow constant variance of errors.

As per QQ plot: It does not seems follow Normal distribution for errors.
...
There were some possible outliers. We removed the outlier with the highest cook’s distance and refit the models.
We found that removing this outlier did not significantly improve model accuracy. In addition, upon further investigation we found that the majority of these high-influence datapoints were not unusual. All the datapoints had high one-day rain totals that were well within the range for Australia’s diverse climates
...
results_table_3 = df_table
knitr::kable(df_table)
```

We also looked at other methods for finding models. We looked at the higher order method for finding models. Noticing that cloud cover played a role in rainfall, we took higher order terms for cloud cover, then ran backward, forward, and stepwise models with AIC and BIC, to try to find the best fit.
...
# Table RMSE, Adjusted R square and LOOCV RMSE
...
library(knitr)
results_table_4 = compare_val
kable(compare_val, padding = 5, caption = "Stats for Best Models")

```

Based on the above table values it seems Best Forward AIC is the best model since it has lowest LOOCV RMSE, RMSE, and highest adjusted R square.
...
results_table_6 = compare_val_step4_alt
...
results_table_7 = compare_val_step5_5
...
sqrt(mean((weather_tst$Rainfall -
             predict(rain_model4, newdata = weather_tst)) ^ 2))
```
...
full_names = c(row.names(results_table_1),
                   row.names(results_table_2),
                   row.names(results_table_3),
                   results_table_4$Model_names, 
                   "best_aic_mod_4_f_alt_trn",
                   "best_aic_mod_4_f_alt_tst",
                   "weather_step_5_aic_trn",
                   "weather_step_5_aic_test")

RSME_loocv = c(results_table_1$RSME_loocv,
               results_table_2$RSME_loocv,
               results_table_3$RSME_loocv,
               results_table_4$Loocv_rmse_vals,
               results_table_6$RMSE_vals_step4_alt,
               results_table_7$RMSE_vals_step5_5)

RMSE_test = c(results_table_1$RMSE_test,
              results_table_2$RMSE_test,
              results_table_3$RMSE_test,
              results_table_4$RMSE_vals,
              results_table_6$RMSE_vals_step4_alt,
              results_table_7$RMSE_vals_step5_5)

adj_r_sqr = c(results_table_1$adjusted_r_squared,
              results_table_2$adjusted_r_squared,
              results_table_3$adjusted_r_squared,
              results_table_4$Adj_r_sqr_vals,
              results_table_6$Adj_r_sqr_vals_step4_alt,
              results_table_7$Adj_r_sqr_vals_step5_5)


results_df = data.frame(row.names = full_names,
                        RMSE_Loocv = RSME_loocv,
                        RMSE = RMSE_test,
                        ADJ_R_Sqr = adj_r_sqr)

kable(results_df, 
      col.names = c("RMSE_Loocv", "RMSE", "Adj R Sqr"), 
      padding = 5, caption="Stats for all decision models")
...
In above table, we have following entries after fitting with TEST data: AIC_refit, BIC_refit, best_aic_mod_4_f_alt_tst and weather_step_5_aic_test. Though we have better RMSE and Adjusted R ^2 for AIC_refit and BIC_refit it has more predictors in the model. Due to that reason we have chosen our model which has less number of predictors and comparable values of RMSE and Adjusted R ^2 in  weather_step_5_aic_test. 

Please note: We have added RMSE value in RMSE_Loocv column for these 4 models in the above table: AIC_refit, BIC_refit, best_aic_mod_4_f_alt_tst and weather_step_5_aic_test.

Our final model is as below:

> lm(Rainfall ~ (Humidity9am + Humidity3pm + Pressure3pm + Evaporation)
                  *RainToday + Pressure9am + I(Cloud9am^2) + I(Cloud9am^3) +                            I(Cloud9am^4), data = weather_trn)
...
### Appendix F: Pairs
This is the multicollinarity pairs chart for the data set. 

```{r pairs, out.width="100%", echo = FALSE}
pairs(weather, pch = ".", col = "dodgerblue")
```
